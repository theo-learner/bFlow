...
class Alignment(object):
    ...
    def dump(self, out=sys.stdout):
        ...
        if not self.rc:
            #out.write("%s\n" % (q))
            qmlen = self.q_end - self.q_pos;
        else:
            #out.write("%s\n" % (q))
            qmlen = (len(self.query) - self.q_pos) - self.q_pos;

        #out.write("%s\n" % m)
        #out.write("%s\n" % (r))
				
        rmlen = self.r_end - self.r_pos;

        qmlen = len(q) - qmlen + len(self.query);
        rmlen = len(r) - rmlen + len(self.ref);
        ...
        for i in xrange(len(q)):
            if(q[i] == '-'):
                ...
                penalty -= 0.75;
        cursim = (float(self.matches) - penalty + wildcard) / float(rmlen);
        out.write("%s\n" % cursim);
